...
router.put(
  ...
  (req, res) => {
    ...
    console.log(`UPDATE gigs SET title="${title}" , details="${details}" , category="${category}" , starting_rate="${starting_rate}" where gig_id="${gigID}";`)
    db.query(`UPDATE gigs SET title="${title}" , details="${details}" , category="${category}" , starting_rate="${starting_rate}" where gig_id="${gigID}";`, (e, r) => {
      if (e) {
        res.status(400).send({
          error: e.message,
        });
        ...
        if (req.files === undefined || req.files.length === 0) {
            ...
            db.query(`DELETE from gigs_images where gig_id="${gigID}"` , (e,r) => {
                if(e) {
                    ...
                    res.status(200).send({ message: "Gig Updated with gig images" });
                } else {
                    console.log("ERROR EH")
                    res.status(400).send({error:"Could not update gig with images"});
                }
            })
        }
      } else {
